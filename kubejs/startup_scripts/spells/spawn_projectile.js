StartupEvents.registry('irons_spellbooks:spells', event => {
    event.create('spawn_projectile')
        ...
        .onCast(ctx => global.spawn_projectile(ctx))       // The function to be called when the spell is cast
...
global.spawn_projectile = (/** @type {Internal.CustomSpell$CastContext} */ ctx) => {
    ...
    player.tell('BOOM')
    let arrow = target.block.createEntity("minecraft:arrow")
    arrow.setPos(target.x, target.y+10, target.z)
    arrow.setMotionY(-2)
    arrow.spawn()
}